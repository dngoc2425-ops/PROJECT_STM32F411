...
void DMA2_Stream0_ADC_Config(unsigned short *buffer)
{
    ...
    DMA2_CONTROL->STREAM[0].PAR  = (unsigned long)&ADC1_CONTROL->DR; // Chon dia chi nguon du lieu
    DMA2_CONTROL->STREAM[0].M0AR = (unsigned long)buffer; // Chon dia chi noi luu du lieu
